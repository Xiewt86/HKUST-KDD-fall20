...
def get_sod_batch(data, profile_data, plates, input_type, num_days = 5, new_days = False, batch=128):
    locs = []
    labels = []
    for _ in range(batch):
        d, l = get_pairs_s_or_d(data, profile_data, plates, input_type, num_days, new_days)
        locs.append(d)
        labels.append(l)
    return locs, labels
...
def get_sad_batch(data, profile_data, plates, input_type, num_days = 5, new_days = False, batch=128):
    locs = []
    labels = []
    for _ in range(batch):
        d, l = get_pairs_s_and_d(data, profile_data, plates, input_type, num_days, new_days)
        locs.append(d)
        labels.append(l)
    return locs, labels
